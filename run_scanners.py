...
RS_LEADER_SCRIPT = os.path.join(BASE_DIR, "rs_leader_radar.py")
...
RS_LEADER_SUMMARY_FILE = os.path.join(OUTPUT_DIR, "rs_leader_radar_summary.json")
...
def main() -> None:
    ...
    if os.path.exists(RS_LEADER_SCRIPT):
        run_status["RS Leader"] = run_script(RS_LEADER_SCRIPT)
    else:
        run_status["RS Leader"] = "스킵(파일 없음)"
    ...
    rs_leader_summary = read_json_file(RS_LEADER_SUMMARY_FILE)
    ...
    lines.append(f"- RS Leader: {run_status.get('RS Leader', '-')}")
    lines.append("")
    ...
    if vcp_summary:
        ...
        lines.append("- 데이터 없음")
    lines.append("")
    lines.append("6) RS 리더 레이더")
    if rs_leader_summary:
        lines.append(f"- 총 후보: {safe_str(rs_leader_summary.get('total', 0))}")
        lines.append(f"- 돌파 준비: {safe_str(rs_leader_summary.get('ready_count', 0))}")
        lines.append(f"- Watchlist: {safe_str(rs_leader_summary.get('watch_count', 0))}")
        lines.append(f"- RS S: {safe_str(rs_leader_summary.get('rs_s', 0))}")
        lines.append(f"- RS A: {safe_str(rs_leader_summary.get('rs_a', 0))}")
    else:
        lines.append("- 데이터 없음")
    lines.append("")
    lines.append("7) 합류 신호")
